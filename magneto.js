...
Magneto.prototype = new Bot();

Magneto.prototype.setup = function() {
...
Magneto.prototype.acquireTarget = function() {
	var target = undefined;
    var enemyHealth = undefined;
...
Magneto.prototype.checkCollisions = function(point) {
	var collision = false;
	var type = '';
...
Magneto.prototype.distanceToCollision = function(x, y, angle) {
	var speed = 5;
...
Magneto.prototype.myDistanceToCollision = function() {
...
Magneto.prototype.run = function() {
	...
	if (this.canShoot && this.weapons.bullet > 0 && (aimingAtEnemy)) {
		...
		if (dir.command != 'forward') {
			...
			// If one of our team bots is in front of us,
			// strafe right to get out from behind them
            //but first try to get a new target
			if (dtc.type == 'bot' && dtc.object.name == this.name) {
				command = 'strafe-right';
			} else {
				// If we're far away, move close
				if (dist > 400 && !aimingAtCloseObstacle) {
					command = 'fire';
				} else {	
					// Otherwise strafe
...
server.registerBotScript("Magneto");